...
mgn_int mainReader(mgn_int argc, mgn_ch** argv) {
	...
	mgn_ch* program = argv[0];
	mgn_ch* input = argv[2];
	mgn_ch mode = MODE_FIXED;
	mgn_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
mgn_void startReader(mgn_ch* program, mgn_ch* input, mgn_ch mode, mgn_int size, mgn_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	mgn_int loadSize = 0;		/* the size of the file loaded in the buffer */
	mgn_ch symbol;			/* symbol read from input file */
...
mgn_void bErrorPrint(mgn_ch* fmt, ...) {
	/* Initialize variable list */	va_list ap;
	va_start(ap, fmt);

	(mgn_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
